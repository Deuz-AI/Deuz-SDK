...
readme_tests=$(grep -oE '\*\*[0-9]+ tests\*\*' "$README" | head -1 | grep -oE '[0-9]+')
intro_tests=$(grep -oE '\*\*[0-9]+ tests\*\*' "$INTRO" | head -1 | grep -oE '[0-9]+')

fail=0

if [ -z "$readme_tests" ] || [ -z "$intro_tests" ]; then
  echo "FAIL  could not find a '**N tests**' mention in README.md and/or docs/content/docs/index.mdx"
  fail=1
...
if [ -n "$actual_tests" ] && [ "$actual_tests" != "$readme_tests" ]; then
