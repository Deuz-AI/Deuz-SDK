...
The package also includes the optional [native agent engine](/docs/modules/native-agents)
and [swarm scheduler](/docs/modules/swarm), and since 2.2 the
[operations](/docs/modules/operations), [persistent budget](/docs/modules/budgets),
[evolve](/docs/modules/evolve) and [schedule](/docs/modules/schedule) modules.
Existing generation APIs remain available. See
[What is new in 2.2](/docs/reference/whats-new-2-2) for upgrade notes.
...
3. **Node adapters use separate subpaths.** A `…/node` suffix is one marker; the `/sqlite` and `/postgres` store subpaths are also Node-only. On an edge runtime, use the web-safe seam plus your own implementation.
...
| `@deuz-sdk/core/swarm/postgres` | Node-only swarm persistence on Postgres through `createPostgresSwarmStore` (2.2) |
| `@deuz-sdk/core/ops` | Leases for runs that several processes may drive: `createInMemoryLeaseProvider` (2.2) |
| `@deuz-sdk/core/ops/sqlite` | Node-only leases, native run storage and budget stores on SQLite (2.2) |
| `@deuz-sdk/core/ops/postgres` | Node-only leases, native run storage and budget stores on Postgres (2.2) |
| `@deuz-sdk/core/evolve` | Evolutionary program search: `evolve`, `resumeEvolve` and an in-memory population store (2.2) |
| `@deuz-sdk/core/evolve/sqlite` | Node-only evolve persistence through `createSqlitePopulationStore` (2.2) |
| `@deuz-sdk/core/schedule` | Cron schedules and verified webhook signals that start or resume runs (2.2) |
...
- [What is new in 2.2](/docs/reference/whats-new-2-2) — release scope, upgrade notes and operational limits.
