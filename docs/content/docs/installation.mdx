...
| --- | --- | --- |
| `zod` (or any [Standard Schema](https://standardschema.dev) lib) + `@standard-community/standard-json` | `generateObject` / `streamObject` / typed tool `parameters` with a schema object | Actionable throw: *"Converting it to JSON Schema needs the optional peer `@standard-community/standard-json` — install it, or pass a raw JSON Schema instead."* Raw JSON Schema needs neither peer. |
...
| `playwright` | [`/browser/node`](/docs/modules/autonomy#browser-control) | Actionable throw: *"requires the optional peer `playwright`. Install it with `npm i playwright && npx playwright install chromium`."* |
...
That is the lazy pump doing its job. `streamChat` sends no request until something reads an output. If you return the result without iterating it — or `await` nothing — the call is a no-op. See [`consume()`](/docs/core/stream-chat#consume).
</Callout>
