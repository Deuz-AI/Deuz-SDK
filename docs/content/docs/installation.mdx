...
Node-only capabilities (filesystem skill sources, document parsers, the markdown memory vault, MCP stdio transport, file workspace, compute sandbox, Playwright browser, JSONL run store) live behind dedicated `…/node` subpaths and never leak `node:*` imports into the edge-safe core.
...
| `playwright` | [`@deuz-sdk/core/browser/node`](/docs/modules/autonomy#browser-control) | Playwright reference adapter for browser control (1.8). Loaded lazily. |
...
| `@deuz-sdk/core/vertex/node` | Application Default Credentials for Vertex (`createAdcKeyProvider`) (1.9) |
| `@deuz-sdk/core/agent` | `createAgent` — a reusable agent as a frozen value (1.9) |
| `@deuz-sdk/core/otel` | OpenTelemetry bridge — `createOtelTracer` / `createOtelObserver` (1.9) |
...
| `@deuz-sdk/core/guardrails` | Built-in guardrails — `promptInjectionGuardrail`, `maxOutputLength` (2.0) |
...
| `@deuz-sdk/core/speech` | Text-to-speech (`generateSpeech`, `createOpenAISpeech`, `createElevenLabs`) (2.0) |
| `@deuz-sdk/core/transcription` | Speech-to-text (`transcribe`, `createOpenAITranscription`, `createDeepgram`) (2.0) |
| `@deuz-sdk/core/video` | Async video jobs (`submitVideo` / `waitForVideo` / `downloadVideo` / `generateVideo`) (2.0) |
...
| `@deuz-sdk/core/mcp/node` | MCP OAuth helpers for Node — `createFileTokenStore`, `createLoopbackRedirect` (2.0) |
| `@deuz-sdk/core/chat` | Framework-neutral chat state/reducers, branching helpers, and the scoped `ChatStore` seam |
| `@deuz-sdk/core/chat/node` | Node JSONL `ChatStore` plus binary-safe chat record codecs |
| `@deuz-sdk/core/stores/sqlite` | `createSqliteStores` — memory / chats / sessions / runs on one `node:sqlite` file (2.0) |
| `@deuz-sdk/core/stores/redis` | `createRedisStores` — memory / chats / sessions over a structural Redis client seam (2.0) |
| `@deuz-sdk/core/stores/postgres` | `createPostgresStores` — four seams, optional pgvector (2.0) |
| `@deuz-sdk/core/durable` | Checkpoint/resume stores, signed approvals, and durable × resumable chat helpers |
| `@deuz-sdk/core/observe` | Local observers, composition/filtering, and pure run summaries for the versioned observation protocol |
| `@deuz-sdk/core/observe/node` | Bounded JSONL observation sink and event reader (Node-only) |
...
| `@deuz-sdk/core/providers` | 19 named OpenAI-compat hosts — Mistral, DeepSeek, Qwen, Moonshot (`createKimi` is an alias), Groq, Together, OpenRouter, Cerebras, Fireworks, GLM, MiniMax + 2.0's Perplexity, Cohere, DeepInfra, NVIDIA, SambaNova, Hyperbolic and the keyless Ollama / LM Studio — plus `createOpenAICompatible`, Azure/Bedrock re-exports and `createProviderRegistry` |
| `@deuz-sdk/core/workspace` | Externalized path-addressed memory seam + `createWorkspaceTools` (1.8) |
| `@deuz-sdk/core/workspace/node` | Sandboxed directory `Workspace` (`createFileWorkspace`) |
| `@deuz-sdk/core/compute` | `ComputeSandbox` seam + `codeActTool` / `shellTool` (1.8) |
| `@deuz-sdk/core/compute/node` | `node:child_process` reference sandbox (`createNodeSandbox`) |
| `@deuz-sdk/core/autonomy` | `planTasks`, `bestOfN`, `selfConsistency`, `parallelAgents`, TaskList reducers (1.8) |
| `@deuz-sdk/core/runtime` | `RunStore` / `createRunManager`, `emitPlanUpdate` / `emitActivity`, steering (1.8) |
| `@deuz-sdk/core/runtime/node` | JSONL run store + `pollStaleRuns` |
| `@deuz-sdk/core/browser` | `BrowserController` seam + `createBrowserTools` (1.8) |
| `@deuz-sdk/core/browser/node` | Playwright reference adapter (`createPlaywrightBrowser`) |
| `@deuz-sdk/core/testing` | Published `createMockModel`, `runEval`, golden-replay fixtures (1.8) |
| `@deuz-sdk/core/ui` | Versioned/resumable Deuz UI wire, stream journaling, reconnect reader, and typed app-data injection |
