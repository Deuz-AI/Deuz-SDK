...
Durable checkpoint/resume has shipped since 1.5. Pass `session: { store, runId }` to either agentic entry point and the loop saves an `AgentCheckpoint` after every completed step boundary. Back `SessionStore` with durable storage (Redis, a database, a Durable Object, and so on) if the checkpoint must survive a process restart; `createInMemorySessionStore()` is only a single-process reference implementation.

```ts title="durable-coding-agent.ts"
import { generateText } from '@deuz-sdk/core';
import { resumeFromCheckpoint } from '@deuz-sdk/core/durable';

// Persist this application-level id with the task/job record.
const runId = 'coding:issue-4821';

const firstLeg = await generateText({
  model: orchestratorModel,
  messages: [{ role: 'user', content: task }],
  tools: { coder },
  maxSteps: 30,
...
// In a later process, after loading the same persistent SessionStore:
const resumed = await resumeFromCheckpoint(sessionStore, runId, {
  model: orchestratorModel,
  tools: { coder },
  maxSteps: 30,
  stopWhen: [totalTokensExceed(500_000), costExceeds(5.0)],
...
A checkpoint is intentionally a **completed-step** recovery unit, not an in-flight transaction. If the process dies while a tool is running, resume re-drives that step, so filesystem and shell tools should still be idempotent or carry their own operation ids. A suspended approval can be continued through `approvalResponses`; with no verdict, pending gated calls default to deny.

Use [`ChatStore`](/docs/modules/chat-persistence) alongside `SessionStore` when you also need a user-facing, unmodified conversation record. Checkpoints preserve the effective execution history needed to continue the model; chat persistence preserves the original conversation plus actual assistant/tool additions.
...
- [Durable runtime](/docs/agents/durable-runtime) — persistent checkpoints, resume semantics, and signed approvals.
