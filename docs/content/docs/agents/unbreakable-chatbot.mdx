...
<Callout title="Why the explicit drain matters here">
...
[`consume()`](/docs/core/stream-chat#consume) takes its own subscription, never rejects, and is memoized, so it is safe alongside the serializer. Use whatever your platform calls the post-response hook: `after()` from `next/server`, or `ctx.waitUntil(...)` inside a Cloudflare Worker `fetch(req, env, ctx)`.
</Callout>
