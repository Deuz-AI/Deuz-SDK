...
interface Tool<Args = unknown, Result = unknown> {
  ...
  timeoutMs?: number; // 1.9: per-execution cap for THIS tool, overrides timeout.toolMs
}
...
| Client tool | A **key present in `tools`** with no `execute` — the loop breaks and the UI does a round-trip | `execute` omitted | `confirmPurchase` (see [Client tools](/docs/agents/client-tools)) |
| Provider tool | The model provider runs it during the turn | `type: 'provider'` + `providerTool` | `anthropicWebSearch()`, `openaiWebSearch()`, `googleSearch()` (see [Provider-executed tools](/docs/agents/server-tools)) |
| Sub-agent tool | A nested agentic loop runs | built by `agentTool(...)` | `agentTool({ name: 'researcher', … })` (see [Sub-agents](/docs/agents/subagents)) |
...
| `timeoutMs` | `number` | no | 1.9. Per-execution cap for this tool, overriding the call's `timeout.toolMs`. Expiry is self-healing: the execution is abandoned and the model gets an `is_error` `tool_result`. See [Timeouts](/docs/core/prompts-and-timeouts#tooltimeoutms). |

The map **key** is the tool name the model calls — there is no `name` field on `Tool` itself.

## The `tool()` helper

`ToolSet` is `Record<string, Tool>`, which erases `Tool<Args, Result>`. A hand-written tool literal therefore gets `args: unknown` inside `execute`, and editing the schema produces **no compile error** in the handler — the mismatch shows up as a runtime failure instead.

```ts title="tool-helper.ts"
...
await generateText({ model, prompt: 'weather in Paris?', maxSteps: 5, tools: { getWeather } });
```

`tool()` is a **pure identity function** — `tool(def) === def`. It imports no validator (zod stays an optional peer), introspects nothing and adds zero runtime behaviour; its entire job is inference. A tool can still be frozen, shared across calls or compared by identity.

A raw JSON Schema has no type-level payload, so it degrades to `unknown` (deliberately **not** `any`): the handler must still narrow, exactly as a hand-written tool does. Two type helpers come with it:

```ts
import type { InferToolInput, InferToolOutput } from '@deuz-sdk/core';

type Args = InferToolInput<typeof getWeather>;  // { city: string }
type Out = InferToolOutput<typeof getWeather>;  // awaited return type
```

Both work on `tool()` results and on plain `Tool<Args, Result>` values; `InferToolOutput` is awaited, so a sync and an async tool report the same type.

<Callout title="One sharp edge">
Calling `myTool.execute(args, ctx)` **directly** checks `args` as `unknown`. `Tool` is invariant in `Args` (the argument sits in a contravariant position), so `tool()`'s return type is an intersection in order to stay assignable to plain `ToolSet` — which is where tools are actually used. The return type is still correct, and authoring inference and the loop are unaffected.
</Callout>
...
## Client tools (a registered key with no `execute`)

A **client tool** is a key **present in `tools`** whose value has no `execute`: the model can request it, but the loop has no way to run it server-side. When a step calls one, the loop stops early and returns the call in `result.toolCalls` — your application owns the round-trip and appends the matching `tool_result` itself.

```ts
tools: {
  // Registered, but no `execute`: the model can request it, you fulfill it client-side.
  openMap: {
...
<Callout type="warn" title="An unregistered name is NOT a client tool (changed in 1.9)">
Before 1.9 a tool call naming something absent from `tools` satisfied the same "no `execute`" test, so a **hallucinated** name broke the loop as if it were a client hand-off — leaving a dangling `tool_use`, no error, and a server waiting forever for a `tool_result` nobody could produce.

An unregistered name is now recognized as unregistered and fed back to the model as an `is_error` `tool_result` in the same turn, and the loop **continues**:

```
No such tool: "search_web". Available tools: getWeather, search.
```

The model usually corrects itself. Unknown-tool errors count toward the runaway guard, so a model looping on the same invented name hard-stops after 3 consecutive failures. A name that collides with an `Object.prototype` member (`toString`, `constructor`) is classified as unknown too — own keys only.
</Callout>
...
```ts title="prepare-step-model-switch.ts"
...
- [createAgent](/docs/agents/create-agent) — bind a tool set to a model once, as a reusable value.
- [Prompts, instructions & timeouts](/docs/core/prompts-and-timeouts) — `timeout.toolMs` and `Tool.timeoutMs`.
