...
A **client tool** is a key **present in your `tools` map** whose value has no `execute` function. The model can still call it, but the SDK can't run it server-side — so the [agentic loop](/docs/agents/tool-loop) stops as soon as a client tool is called and returns the pending tool call to you. The caller owns the round-trip: collect a user confirmation, hit a browser-only API, or run a human-in-the-loop approval, then append a `tool_result` and call again. Use this whenever the work can't (or shouldn't) happen on the server inside the loop.

## How it works

Every tool in a `ToolSet` is either a **server tool** (has `execute`, the loop runs it) or a **client tool** (registered, no `execute`). Two things that look similar but are not client tools:

- A [provider-executed tool](/docs/agents/server-tools) (`type: 'provider'`) has no `execute` but is run by the provider, so it does **not** break the loop.
- An **unregistered** name — one the model invented — is not a client tool either. Since 1.9 it self-heals into an `is_error` `tool_result` and the loop continues. See the callout below.
...
<Callout type="warn" title="A hallucinated name is no longer a client hand-off (1.9)">
Before 1.9 the test was effectively `!tools[name]?.execute`, so a tool call naming something **absent** from `tools` looked exactly like a client tool: the loop exited, returned a turn with a dangling `tool_use` and no error, and a server waiting to send that `tool_result` back waited forever.

An unregistered name is now classified as unknown and fed back as an `is_error` `tool_result` in the same turn — `No such tool: "search_web". Available tools: getWeather, search.` — and the loop **continues**, so the model can correct itself. Own keys only, so `toString` / `constructor` are unknown too. Real client tools are unchanged: a registered key with no `execute` still breaks the loop with no fabricated result.
</Callout>
...
export async function POST(req: Request): Promise<Response> {
  // A client-tool route MUST allow `role: 'tool'` turns — that is the round-trip.
  // Read what that accepts: /docs/modules/request-validation#client-tools-need-rejecttoolresults-false
