...
`sub-agent` parts also cross the [Deuz UI wire](/docs/modules/ui-streaming) and, since 1.9, `applyUIPart` folds each one into `turn.subAgents` (one frame per `agentPath`, each carrying the child's own full `AssistantTurnState`) — so a delegated run is renderable in a `useChat` UI without reading the wire by hand. See [rendering a sub-agent run](/docs/modules/react-hooks#rendering-a-sub-agent-run).
...
> **Approval is not sandboxing.** Gating a `runShellCommand`/`writeFile`/`applyPatch` tool with `needsApproval` decides *whether* it runs — it does nothing to contain *what* it does. For shell/file/code tools, also: run inside a sandbox or a restricted workspace root, enforce allow/deny lists, pass `ctx.signal` into long-running commands, capture `stdout`/`stderr` separately, cap output size before feeding it back to the model, and never expose host secrets or unrestricted filesystem access. See the [coding-agent cookbook](/docs/cookbooks/coding-agent) for the full pattern.

> **Client-mode approval needs a durable session.** Client-mode approval — omitting `approveToolCall` and breaking the loop into `pendingApprovals` — works inside a sub-agent **when the parent call carries `session`** (1.5): the sub-agent suspends into its own checkpoint, the parent suspends with `agentPath`-tagged pending approvals, and resuming the parent routes the verdicts back down the tree. See [Durable runtime](/docs/agents/durable-runtime#client-mode-approval-inside-sub-agents). Without a `session`, a gated sub-agent tool call with no inherited server-mode approver still returns a clear `is_error` explaining this (the parent model can react to it), rather than hanging or silently allowing the call. Server-mode `approveToolCall` on the parent call works at any depth either way.
