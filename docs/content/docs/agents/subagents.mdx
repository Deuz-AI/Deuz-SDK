...
AI SDK's equivalent is the documented "agent-as-tool" pattern: a tool whose `execute` calls another agent and returns its final text. It's a **black box** — you cannot see the sub-agent's internals stream through the canonical stream without hand-writing UI-writer plumbing yourself, and their docs have a dedicated section titled *"No Tool Approvals in Subagents"* because their subagents cannot use tool approval at all. `agentTool` closes both gaps: the sub-agent's entire canonical stream forwards **live** into the parent's `fullStream`, and the parent's approval gate is **inherited** to every nesting depth — with no new runtime, since `execute` just drives the same `streamChat` loop internally.
