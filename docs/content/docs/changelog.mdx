...
**The things you used to build yourself.** Nine new subpaths, three new modalities, real databases behind the four storage seams, guardrails and handoffs in the loop, and MCP that connects itself. Everything additive: no existing type gained a required field, every 1.9 call still compiles, every stored checkpoint still loads. Full surface **and its limits**: [What is new in 2.0](/docs/reference/whats-new-2-0).
...
- **Compaction** — public `compactMessages()`, a rolling summary that folds instead of stacking, automatic recovery from a provider context-overflow rejection, and `countTokens` for a real tokenizer. → [Compaction](/docs/modules/compaction)
