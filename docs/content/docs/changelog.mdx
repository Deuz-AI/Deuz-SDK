...
- **Durability operations** (`/ops`, `/ops/sqlite`, `/ops/postgres`,
  ...
  `handle.drain()`, `swarm.recover()`, `swarm.requestCancel(key)`,
  `expectedRevision` / `expectedStatus` on resume, a revision-gated Postgres
  swarm store, and revision-fenced native agent run stores.
...
- **Persistent budgets** — `admission: { store, scopes, warnAtPercent, onWarning }`
  ...
  `user:42`, with rolling windows. Memory, SQLite and Postgres budget stores.
...
- **Evolve** (`/evolve`, `/evolve/sqlite`) — evolutionary program search over
  ...
  islands, a MAP-Elites archive, a UCB1 model bandit, a mandatory budget and
  zero-call resume. → [Evolve](/docs/modules/evolve)
