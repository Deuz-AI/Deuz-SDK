...
<Callout type="info" title="For production, use a store pack">
The two stores below are references: a `Map` and one JSON file per chat. 2.0 ships real backends — [`createSqliteStores`](/docs/modules/stores#sqlite), [`createRedisStores`](/docs/modules/stores#redis) and [`createPostgresStores`](/docs/modules/stores#postgres) — each returning a `ChatStore` alongside the matching `SessionStore` / `MemoryStore` on the same connection, with the binary-safe record codec already wired.
