...
<Callout type="warn">
  Identity-based keys are why a **new object per request** is a leak: build the value once, at module scope. A config carrying a fresh elicitation handler on every call mints a fresh connection every call.

  The pool bounds that rather than trusting it: `createMcpPool({ maxSize })` defaults to **32** entries and evicts the least recently used one, closing it. Pass a `logger` to hear about an eviction — a pool that churns is telling you a key varies per request.
</Callout>
