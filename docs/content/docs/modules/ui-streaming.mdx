...
| `cost` **(v2)** | `costUsd`, `deltaUsd?`, `cacheSavingsUsd?`, `stepIndex?` | Live cumulative USD cost, one per step when `deps.priceProvider` is injected. See [Pricing](/docs/modules/pricing#live-cost-stream). |
