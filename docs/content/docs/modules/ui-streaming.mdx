...
- **v2** frames each event as `id: <seq>\ndata: <json>\n\n` and may carry the v2-only part types (`data-{name}`, `citation`, `tool-state`, `cost`, `budget-exceeded`).
...
Tip: instead of accumulating `tool-input-delta` fragments and inferring status from part ordering, wire v2 lets you render tool lifecycle directly from `tool-state` parts — and the pure reducer `applyUIPart` (`@deuz-sdk/core/chat`) folds *all* of these parts, including `data-{name}`, `citation`, `cost`, and `budget-exceeded`, into one immutable turn state for you. See [Chat persistence & state](/docs/modules/chat-persistence).
