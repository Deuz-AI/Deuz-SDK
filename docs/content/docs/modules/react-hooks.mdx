...
> **1.7:** the React surface moved to its own package, **`@deuz-sdk/react`** — `useChat` v2 (chatId, rendered `initialMessages`, live `cost`/`budgetExceeded` state, `regenerate`/`editAndResend`, signed-approval token flow, `reconnect()` over `connectDeuzStream`) plus headless `ToolApprovalCard` and `CostBadge` components. Everything below — the `@deuz-sdk/core/react` subpath — keeps working but is **frozen**; new features land in `@deuz-sdk/react`, whose hooks bind the pure chat engine at [`@deuz-sdk/core/chat`](/docs/modules/chat-persistence).
