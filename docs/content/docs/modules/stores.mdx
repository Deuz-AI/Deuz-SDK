...
- **A failed open is retried.** Redis forgets a rejected connect (guarded by promise identity, so a later success is never dropped), Postgres clears a failed `migrate()`, and since 2.2 SQLite forgets a failed open too, so a transient outage — a locked file, a directory created a moment later — does not poison the pack for its lifetime. A handle SQLite opened itself is closed after a failed open; an injected `{ database }` stays open for the retry. Before touching the file, SQLite sets a 5 s `busy_timeout` (unless an injected handle already has one), so it waits for another process's lock instead of failing at once.
...
- **The whole batch is one multi-statement query wrapped in `BEGIN … COMMIT`.** That is not stylistic: `pool.query('BEGIN')` followed by a separate `query(ddl)` can land on different pooled connections and silently run the DDL outside the transaction. Since 2.2 the batch takes a transaction-scoped advisory lock first and a failed batch rolls back, so processes starting together against an empty schema queue instead of failing with a duplicate-type error, and a lost race (`23505`, `42P07`, `42710`, `40001`) is retried.
