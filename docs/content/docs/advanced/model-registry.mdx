...
When a slug is not in the registry, the orchestrator builds a conservative fallback row and calls `logger.warn` (route a logger in through [dependencies](/docs/core/dependencies) to see it). Since 1.9 it also records a typed `{ type: 'unknown-model' }` [`CallWarning`](/docs/core/stream-chat#warnings) — readable on a tool-less `streamChat`'s `warnings` and as a `warning` part on `fullStream`, still exactly one log line. The fallback differs by surface so the failure mode is always "degraded but working," never a crash:
