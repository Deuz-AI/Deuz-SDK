...
The core touches only APIs that exist in every modern JS runtime: `fetch`, Web Streams (`ReadableStream`/`TransformStream`), `TextEncoder`/`TextDecoder`, `WebCrypto`, and `atob`/`btoa`. Anything stateful or non-deterministic is injected through the single [`Dependencies`](/docs/core/dependencies) seam instead of being read from the ambient environment — including the HTTP transport (`fetch`), the clock, id generation, logging, tracing, and key resolution.
