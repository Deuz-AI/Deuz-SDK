...
description: One key, one base URL — chat, image, video, embeddings, and Midjourney across a 2026 multi-vendor catalog.
---

Yunwu (云雾) is an OpenAI-compatible aggregator: a single `apiKey` + `baseURL` fronts hundreds of upstream models from many vendors (OpenAI, Anthropic, Google, xAI, DeepSeek, Qwen, …). `createYunwu` is the **unified client** — you supply the host once and it derives the right surface per call: chat / image / video / embeddings under `/v1`, and the Midjourney proxy at the bare host root. Reach for a relay when you want a **single key**, **mainland-China access**, and **broad model variety** without wiring each provider separately.
...
| Video (async) | `video(modelId)` | `{root}/v1/videos` | `video` |
| Embeddings | `embedding(modelId)` | `{root}/v1/embeddings` | `openai-embeddings` |
| Midjourney (async) | `mj()` | `{root}/mj/...` (bare root, **not** `/v1`) | — |

The "creative base URL" rule: point `baseURL` at any mirror or self-host and **all five** surfaces follow it.
...
mirror.video('sora-2'); //       → https://my-mirror.example.com/v1/videos
...
| `video(modelId)` | `VideoModel` | For [`generateVideo`](/docs/modules/video-generation) / `submitVideo`. |
...
const video = createYunwuVideo({ apiKey: process.env.YUNWU_API_KEY! }); // VideoProvider
...
video('veo3.1'); //         VideoModel     (surface: 'video')
...
YUNWU_MODELS.video; //      async video slugs (submit → poll → download)
...
| Video (async) | `YUNWU_VIDEO_MODELS` | `sora-2`, `veo3.1`, `kling-2.6`, `viduq3-pro`, `MiniMax-Hailuo-2.3` |
| Midjourney | `YUNWU_MIDJOURNEY_MODELS` | `mj_imagine`, `mj_variation`, `mj_upscale`, `mj_blend`, `mj_describe` |

The model-list types (`YunwuChatModel`, `YunwuImageModel`, `YunwuVideoModel`) accept any `string`, so `chat('some-new-slug')` type-checks while autocompleting the pinned catalog.
...
## Example: generateVideo

The video surface is **async** — `POST /v1/videos` creates a job, the SDK polls it, and `download: true` pulls the finished clip's bytes from `/v1/videos/{id}/content`. Pass `yunwu.video(slug)` to [`generateVideo`](/docs/modules/video-generation):
...
const { task } = await generateVideo({
  ...
  prompt: 'a tiny robot watering a bonsai, slow dolly in',
  size: '1280x720',
  seconds: 8,
  onProgress: (t) => console.log(t.status, t.progress),
});

console.log(task.status, task.url); // 'completed' https://…/clip.mp4
```

A job the model refuses resolves with `status: 'failed'` and a `failReason` — it does **not** throw. The relay path is a setting, not a constant: if a host mounts its video jobs somewhere other than `{root}/v1/videos`, build the descriptor with `createVideoProvider({ baseURL })` instead (see [Video generation](/docs/modules/video-generation)).
...
- [Video generation](/docs/modules/video-generation) — the async submit → poll → download job surface.
