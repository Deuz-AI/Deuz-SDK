...
The canonical `effort` option (`'none' | 'low' | 'medium' | 'high'`) is passed through to the Responses request as `reasoning: { effort }`, but only when the model is reasoning-capable and `effort` is not `'none'`:

| `effort` value | Responses request |
| --- | --- |
| `'none'` | `reasoning` omitted (no reasoning block sent). |
| `'low'` / `'medium'` / `'high'` | `reasoning: { effort: <value> }`. |
