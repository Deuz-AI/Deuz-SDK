...
<Callout type="warn" title="Five claims in this section were wrong and have been corrected">
...
| `GenerateObjectResult` | A per-call sink threaded through `getCapabilities` and the inner `runStream`. |
| `StreamObjectResult` | A promise resolved from the same sink at the terminal boundary. |
| `StreamChatResult` | A promise, plus live `warning` parts on `fullStream`. |
