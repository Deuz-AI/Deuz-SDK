...
description: The full 2.0 surface — persistent stores, guardrails, handoffs, zero-config MCP with OAuth, three new modalities — plus the four loud breaks and the limits stated plainly.
---
...
The public 1.9 call surface still compiles. No existing type gained a required field, and every stored checkpoint still loads. That is the contract. Four **loud** breaks sit next to it — a dead error class, two unions that grew a member, a stream part that used to omit a field, and three more loop options that structured output now refuses instead of ignoring. They are listed in [Breaking changes](#breaking-changes), not buried in a changelog.

Two behaviours also changed, both in the recovering direction: a context-overflow rejection is compacted and retried instead of killing the run, and a dead HTTP MCP session is noticed. See [Behaviour changes](#behaviour-changes).
...
| You… | Do this |
| --- | --- |
| Import `NotImplementedError` | Delete the import. It was never thrown. |
| `switch` exhaustively on `ObservedSubsystem` | Add `speech`, `transcription`, `video`. A `default` branch is enough. |
| `switch` exhaustively on compaction `trigger` | Add `'manual'` and `'overflow'`. Absent still means `'threshold'`. |
| Read `compaction` parts off `fullStream` | `trigger` is now always present. Treat missing as `'threshold'` if you still have a 1.9 producer in the mix. |
| Pass `mcp`, `guardrails`, or `doneWhen` to `generateObject` / `streamObject` | Those calls now fail fast, the same way 1.9 made `tools` / `maxSteps` loud. Run the loop with `generateText` / `streamChat`, then structure the text. |
| Catch a generic `InvalidRequestError` around a too-long prompt | The run may now **succeed** after one forced compaction pass (Anthropic, Chat Completions, Responses). Gemini `native` is unchanged — set `compaction` explicitly there. |
| Depend on an HTTP MCP session staying `connected` after the socket dies | 2.0 pings once and counts a failed ping as a drop, so `reconnect: true` can fire without `keepAliveMs`. |
...
| You want | Start here |
| --- | --- |
| Chat / memory / session / runs on a real database | [Persistent stores](/docs/modules/stores) |
| Block or rewrite input, tool calls, or the final answer | [Guardrails](/docs/agents/guardrails) |
| Transfer the run to another agent | [Handoffs](/docs/agents/handoffs) |
| MCP servers without a manual client | [MCP](/docs/modules/mcp) |
| Survive turn forty, or recover from a context-overflow 400 | [Compaction](/docs/modules/compaction) |
| TTS / STT / video | [Speech](/docs/modules/speech) · [Transcription](/docs/modules/transcription) · [Video](/docs/modules/video-generation) |
| Ollama, LM Studio, or a new cloud host | [Local models](/docs/providers/local) · [OpenAI-compat hosts](/docs/providers/compat) |
...
**2. Two unions gained members.** `ObservedSubsystem` now includes `'speech' | 'transcription' | 'video'`. The compaction `trigger` union now includes `'manual' | 'overflow'` (plus the pre-2.0 `'threshold'`). An exhaustive `switch` over either needs a new case — or a `default`, which is what [the stream protocol](/docs/reference/stream-protocol) already asks you to keep.
...
- [Changelog](/docs/changelog) — the full release history, including 1.9 and earlier.
- Jump back to [Upgrading from 1.9](#upgrading-from-1-9) if you are bumping a running app.
