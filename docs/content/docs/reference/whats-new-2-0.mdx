...
2.0 is the release where the things you had to build yourself became things you configure. Nine new subpaths, three new modalities, real databases behind the four storage seams, guardrails and handoffs in the loop, and MCP that connects itself.
...
Plus new root exports: `handoff`, `compactMessages`, `promptInjectionGuardrail`, `maxOutputLength`, `McpAuthorizationRequiredError`, `ContextOverflowError`.
...
`@deuz-sdk/core/providers` gained **Perplexity, Cohere, DeepInfra, NVIDIA NIM, SambaNova, Hyperbolic** (cloud) and **Ollama, LM Studio** (local, keyless). That is 29 provider factories in total.
...
**1. A context-overflow rejection is recovered, not fatal.** On the `anthropic` and `chat_completions` wires an over-long request now maps to `ContextOverflowError`, and the loop force-compacts and retries the step once — even with no `compaction` option set. Before 2.0 it ended the run. If you were catching a generic `InvalidRequestError` around that case, you will now see the run succeed instead.
...
- **Overflow recovery only fires on two wires.** `ContextOverflowError` is mapped by the `anthropic` adapter (HTTP 413, or a 400 whose *message* matches — Anthropic ships no machine-readable code) and by the `chat_completions` adapter (`context_length_exceeded`). On the **`responses`** and **`native` (Gemini)** wires an over-long request is still a generic error and the run ends. Set `compaction` explicitly there; the threshold path does not depend on error mapping.
...
- **Compaction summaries and pricing use the root model after a handoff.** The active agent drives the steps; the summarize side call still uses `summarizeModel ?? options.model`, and cumulative cost is priced against `options.model.modelId`.
