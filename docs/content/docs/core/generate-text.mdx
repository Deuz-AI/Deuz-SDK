...
| `messages` | `Message[]` | — | Canonical message history. Required unless you pass `prompt` — the two are mutually exclusive, and passing both (or neither) rejects with an `InvalidRequestError`. |
...
| `signal` | `AbortSignal` | — | Cancellation; propagated to the underlying fetch and to tool `execute`. `abortSignal` is a deprecated 1.9 alias. |
| `maxRetries` | `number` | `2` | Pre-first-byte retries only. |
| `timeout` | `number \| { ttftMs?, totalMs?, stepMs?, toolMs? }` | ttft 60s / total 300s | **1.9.** A bare number is `{ totalMs }`; `0` disables a layer. Note `stepMs` is enforced in the **streaming** loop only — this buffered loop honours `ttftMs`/`totalMs`/`toolMs`. See [Timeouts](/docs/core/prompts-and-timeouts#timeout). |
...
| `warnings` | `CallWarning[]` | when the call produced any | Non-fatal notices — an unknown model slug, a stripped sampling parameter, a dropped document, a hosted tool the wire cannot carry, an `activeTools` name that matches nothing. **Omitted entirely** when there are none, so a clean result's shape is unchanged. On an agentic call the per-step pump writes into the loop's sink, so model-level notices merge in too. Deduplicated by `(type, setting, message)` and capped at 50. |
...
- **An unregistered tool name is not a client tool** (1.9): a hallucinated name self-heals into an `is_error` `tool_result` and the loop **continues**, instead of breaking out with a dangling `tool_use`.
- **Runaway guard:** the same tool failing on three consecutive steps hard-stops the loop. Unknown-tool errors and tool timeouts count toward it; approval denials do not.
