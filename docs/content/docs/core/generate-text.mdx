...
| `warnings` | `CallWarning[]` | **never (1.9)** | `CallWarning` has real producers, but **not on this result**: a buffered call raises and logs its notices and then discards them — no step or loop code puts them on the returned object, so `result.warnings` is `undefined` on every real `generateText`. Read `deps.logger.warn`, or use [`streamChat`](/docs/core/stream-chat#warnings), where the field is live. See [Declared but inert](/docs/reference/whats-new-1-9#declared-but-inert). |
