...
//   warnings?: Promise<CallWarning[]>;   // resolves with the run's notices, `[]` when clean
// }
```
...
`warnings` is a promise resolved from a per-call sink at the terminal boundary — the same shape [`streamChat`'s `warnings`](/docs/core/stream-chat#warnings) has, and it never rejects. What it cannot do is reach a `useObject` client: `toDeuzObjectStreamResponse` iterates `partialObjectStream`, and `StreamObjectResult` exposes no canonical part stream, so there is nothing to serialize a `warning` part from. Read it server-side.

<Callout type="warn" title="Loop options are refused (1.9, extended in 2.0)">
Structured output is single-turn. Passing `tools`, `maxSteps > 1`, `stopWhen`, `verifyStep`, `memory`, `session`, … used to be accepted and then ignored; it now fails. 2.0 adds `mcp`, `guardrails`, and `doneWhen` to that list. `streamObject` reports it through its never-throw shape — one `error` part, rejecting `partialObjectStream`, and rejected `object` / `usage` / `finishReason`. See [generateObject](/docs/core/generate-object#loop-options-are-refused).
</Callout>
