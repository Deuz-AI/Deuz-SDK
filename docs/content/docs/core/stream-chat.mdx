...
The buffered readouts carry the same set: `GenerateTextResult.warnings`, `GenerateObjectResult.warnings` and `StreamObjectResult.warnings` are populated too, and are **omitted entirely** when a call produced none, so a clean result's shape is unchanged.
