...
- **Overflow auto-recovery** — a provider that rejects a request as too long triggers a forced compaction pass and one retry, even when `compaction` was never set. It fires on the `anthropic` and `chat_completions` wires only.
