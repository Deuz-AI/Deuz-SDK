...
interface CompactionPolicy {
  ...
  countTokens?: (messages: Message[]) => number; // 2.0 — a real tokenizer
}
...
| `countTokens` | `(messages) => number` | — (char heuristic) | 2.0. A real tokenizer, used as the BASE count; the EMA calibration still runs on top of it. Must be synchronous. See [the recipe](/docs/modules/compaction#a-real-tokenizer-via-counttokens). |
...
interface CompactionPart {
  ...
  tokensBefore: number;
  tokensAfter: number;
  /** 2.0 additive; absent = 'threshold', the only trigger before 2.0. */
...
## Beyond the automatic policy

2.0 adds three things this page does not cover, all on [Compaction & context recovery](/docs/modules/compaction):

- **`compactMessages()`** — the same layers as a plain function over a `Message[]` you own. Pure by default: with no summarizer it makes no network call at all.
- **The rolling summary** — pass N folds into pass N−1's summary instead of restarting, so at most one summary block ever sits in the history.
- **Overflow auto-recovery** — a provider that rejects a request as too long triggers a forced compaction pass and one retry, even when `compaction` was never set. It fires on every chat wire whose adapter recognizes the rejection: `anthropic`, `chat_completions`, and `responses` (which reuses the `chat_completions` error mapping). The Gemini `native` wire does not map it.

## See also

- [Compaction & context recovery](/docs/modules/compaction) — `compactMessages`, the rolling summary, overflow recovery, `countTokens`.
