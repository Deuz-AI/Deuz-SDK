...
The agentic loop is bounded the same way as the AI SDK: `maxSteps` (default **1**, i.e. single-turn) plus an optional `stopWhen` predicate, OR-ed together. There are no exported `stepCountIs` / `hasToolCall` helpers — `stopWhen` is a plain callback you write inline, receiving `{ steps, stepCount }`.
