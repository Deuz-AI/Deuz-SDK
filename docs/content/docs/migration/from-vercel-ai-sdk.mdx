...
The AI SDK auto-reads `process.env.ANTHROPIC_API_KEY` (and friends) inside the provider. `@deuz-sdk/core` never reads `process.env` or logs through `console`; it takes keys, logging, clock, and id sources through replaceable seams (with documented host clock/id defaults). Read the key at your app layer and pass it into the factory.
