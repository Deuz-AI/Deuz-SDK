...
| `useChat` (React hook) | `readDeuzStream` (manual) | Hooks are planned (Faz 6); read the stream yourself today. |
...
- **React hooks** (`useChat`, `useCompletion`, `useObject`) — planned for Faz 6 (`@deuz-sdk/core/react`). For now, drive [`readDeuzStream`](/docs/modules/ui-streaming) yourself.

Everything else in the chat, tools (including approval), structured output (buffered and streaming), embeddings, MCP, and UI surface has a direct equivalent above.
