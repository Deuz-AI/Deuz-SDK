...
| `streamObject` | — | Not implemented yet. |
...
On a parse/validation miss `@deuz-sdk/core` runs one repair retry, then throws `NoObjectGeneratedError`. Note: there is no `streamObject` yet (see below).
...
- **`streamObject`** — only the buffered [`generateObject`](/docs/core/generate-object) is available. Stream the text and parse client-side if you need partial objects.
...
Everything else in the chat, tools, embeddings, and UI surface has a direct equivalent above.
