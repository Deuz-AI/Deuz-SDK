...
This table is the **common base subset**. When you pass `tools`, the agentic loop unlocks more options — summarized here, each documented in full on [Tool calling](/docs/agents/tools):
...
| Long context | `compaction` — automatic [layered compaction](/docs/core/compaction) |
...
> **`effort` mapping.** `xhigh` and `max` are canonical values; each adapter maps or clamps them to the closest provider-supported level. OpenAI clamps `max` → `xhigh`; Gemini maps both toward its high/native-budget setting; newer Anthropic models (Opus 4.7+, Sonnet 5, Fable 5) accept them directly on the `output_config` effort wire. See each [provider page](/docs/providers/anthropic) for the exact per-model behavior.
...
[`validateChatRequest`](/docs/modules/request-validation) is a pure structural gate: it never repairs, and it rejects an injected `role: 'system'` turn, a forged `tool_result`, and an oversized history. A route serving **client tools** must pass `{ rejectToolResults: false }`, because the browser legitimately POSTs a `role: 'tool'` message.
