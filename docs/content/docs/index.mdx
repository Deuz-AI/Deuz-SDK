...
Most AI SDKs read environment variables, reach for `Date.now()` / `Math.random()`, and proxy a provider's raw bytes straight to your client. That makes them impure (hard to test), bound to one runtime, and locked to whatever shape the upstream provider chose. `@deuz-sdk/core` inverts all of that.
...
- **207 tests** across 26 files (vitest with golden-replay SSE fixtures and deterministic mock models — no real network)
