...
| **An agent with validated final results** | [Native agents](/docs/modules/native-agents) → [What is new in 2.1](/docs/reference/whats-new-2-1)                                               |
...
| **A multi-agent system**                  | [Sub-agents](/docs/agents/subagents) → [Handoffs](/docs/agents/handoffs) → [Durable swarm DAGs](/docs/modules/swarm)                             |
...
2.2 lets swarms grow while they run, moves durable runs between processes, and
adds persistent budgets, evolutionary search and schedules. The 2.1 APIs remain
available.

- **Dynamic swarms** — finished tasks spawn tasks atomically, with limits,
  compensation and per-task timeouts → [Swarm](/docs/modules/swarm)
- **Blackboards and rounds** — group channels agents read and post to, soft
  dependencies and explore-consolidate rounds.
- **Operations** — leases, drain, cross-process cancel and recovery, with SQLite
  and Postgres backends → [Operations](/docs/modules/operations)
- **Persistent budgets** — per-user or per-org scopes shared by every run and
  process → [Budgets](/docs/modules/budgets)
- **Evolve** — evolutionary program search over islands with a mandatory budget
  and zero-call resume → [Evolve](/docs/modules/evolve)
- **Schedules and signals** — cron ticks and verified webhooks that start or
  resume runs → [Schedules](/docs/modules/schedule)

Start with [What is new in 2.2](/docs/reference/whats-new-2-2) for upgrade notes and
operational limits. Previous releases: [2.1](/docs/reference/whats-new-2-1),
[2.0](/docs/reference/whats-new-2-0) and [1.9](/docs/reference/whats-new-1-9).
