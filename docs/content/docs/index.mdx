...
2.1 adds an optional native agent engine and a durable swarm scheduler. Existing
generation and `createAgent` APIs remain available; native results, checkpoints
and events have their own contracts.

- **Validated agent results** — `runAgent` / `streamAgent`, explicit outcomes,
  structured finalization and three-outcome verification → [Native agents](/docs/modules/native-agents)
- **Strict persistence** — native checkpoints, approval/client-result resume and
  tool receipts that require reconciliation for uncertain external effects.
- **Durable swarm DAGs** — bounded concurrency, reducers, memory/SQLite stores,
  cancellation, completed-work reuse and cursor events → [Swarm](/docs/modules/swarm)
- **Shared policy and budgets** — inherited constraints, reservations before
  admission, and retained unknown amounts across recovery.

Start with [What is new in 2.1](/docs/reference/whats-new-2-1) for upgrade notes and
operational limits. Swarms use one executor per run, cancellation is cooperative,
and budget admission uses estimates. Previous releases: [2.0](/docs/reference/whats-new-2-0)
and [1.9](/docs/reference/whats-new-1-9).
