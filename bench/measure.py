...
def install_spec(pkg):
    """Return the npm install argument for a panel package."""
    if pkg == "@deuz-sdk/core" and DEUZ_TARBALL:
        path = os.path.abspath(DEUZ_TARBALL)
        if not os.path.isfile(path):
            raise FileNotFoundError(f"DEUZ_TARBALL not found: {path}")
        return path
    return pkg
...
def main():
    results = []
    for pkg in PACKAGES:
        spec = install_spec(pkg)
        label = f"{pkg} (local tarball)" if spec != pkg else pkg
        print(f"[{label}] installing…", flush=True)
        ...
        try:
            ...
            version = "?"
            try:
                with open(os.path.join(nm, *pkg.split("/"), "package.json"), encoding="utf-8") as fh:
