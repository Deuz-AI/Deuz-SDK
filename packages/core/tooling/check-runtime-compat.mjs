...
for (const consumer of consumers) {
  try {
    ...
    const nodeOnlyInput = Object.keys(result.metafile?.inputs ?? {}).find((input) =>
      /(?:rag-node|memory-markdown|skills[\\/]node|mcp[\\/]stdio|node[\\/](?:observe|chat-store|workspace|compute|browser|runtime|vertex-auth|mcp|store-sqlite|store-redis|store-postgres))/.test(
        input,
      ),
    );
